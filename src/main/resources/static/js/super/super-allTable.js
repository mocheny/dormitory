    ...
    $.get("/allStuQuery", {
        ...
        for (let i = 0; i < page.records.length; i++) {
            ...
            let tr = '<tr>\n' +
                ...
                '                           data-target="#as-update' + student.sid + '" onclick="javascript:as_show_update_modal(' + student.sid + ')">修改</button>\n' +
                '                    <button type="button" class="btn btn-primary btn-danger" data-toggle="modal" ' +
                '                           data-target="#as-delete' + student.sid + '" onclick="javascript:as_show_delete_model(' + student.sid + ')">删除</button>\n' +
                '                </td>\n' +
                '            </tr>';
            table += tr;
        }
...
/////////////////////////////////////////////////AllDormBuildings//////////////////////////////////////////////////////////
function adb_load_table(current, searchContent) {
    $.get("/allDbQuery", {current: current, searchContent: searchContent}, function (page) {
        if (searchContent === null || searchContent === undefined) {
            searchContent = '';
        }

        //设置数据量相关信息
        $('#adb-current').html(page.current);
        $('#adb-pages').html(page.pages);
        $('#adb-total').html(page.total);
        //设置数字浮标
        $('#adb-count').html(page.total);

        let lis = "";
        if (page.pages > 0) {
            let beforeNum = page.current === 1 ? page.current : page.current - 1;
            let beforePage = '<li onclick=\'javascript:adb_load_table(' + beforeNum + ',"' + searchContent + '")\'><a href="javascript:void(0)" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>';
            let nextNum = page.current === page.pages ? page.current : page.current + 1;
            let nextPage = '<li onclick=\'javascript:adb_load_table(' + nextNum + ',"' + searchContent + '")\'><a href="javascript:void(0)" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>';
            lis += beforePage;
            let begin;
            let end;
            if (page.pages < 10) {
            ...
            for (let i = begin; i <= end; i++) {
                let li = '';
                if (page.current === i) {
                    li = '<li class="active" onclick=\'javascript:adb_load_table(' + i + ',"' + searchContent + '")\'><a href="javascript:void(0)">' + i + '</a></li>';
                } else {
                    li = '<li onclick=\'javascript:adb_load_table(' + i + ',"' + searchContent + '")\'><a href="javascript:void(0)">' + i + '</a></li>';
                }
                lis += li;
            }
            lis += nextPage;
        }
        $('#adb-pagination').html(lis);
        ...
        let table = '<tbody>' +
            ...
            '            </tr>';
        for (let i = 0; i < page.records.length; i++) {
            let dormBuilding = page.records[i];
            let tr = '<tr>\n' +
                '                <td>' + dormBuilding.dbname + '</td>\n' +
                '                <td>' + dormBuilding.roomNumber + '</td>\n' +
                '                <td>' + dormBuilding.studentNumber + '</td>\n' +
                '                <td>\n' +
                '                    <button type="button" class="btn btn-primary btn-success" data-toggle="modal" ' +
                '                           data-target="#adb-update' + dormBuilding.dbid + '" onclick="adb_show_update_modal(' + dormBuilding.dbid + ')">修改</button>\n' +
                '                    <button type="button" class="btn btn-primary btn-danger" data-toggle="modal" ' +
                '                           data-target="#adb-delete' + dormBuilding.dbid + '" onclick="adb_show_delete_model(' + dormBuilding.dbid + ')">删除</button>\n' +
                '                </td>\n' +
                '            </tr>';
            table += tr;
        }
        table += '</tbody>';
        $('#adb-table').html(table);
        tableFormatting();  //表格格式化
    });
}
...
function adm_load_table(current, dormBuildingName, searchContent) {
    $.get("/allDManQuery", {
        current: current,
        dormBuildingName: dormBuildingName,
        searchContent: searchContent
        ...
        //设置数据量相关信息
        $('#adm-current').html(page.current);
        $('#adm-pages').html(page.pages);
        $('#adm-total').html(page.total);
        //设置数字浮标
        $('#adm-count').html(page.total);

        let lis = "";
        if (page.pages > 0) {
            let beforeNum = page.current === 1 ? page.current : page.current - 1;
            let beforePage = '<li onclick=\'javascript:adm_load_table(' + beforeNum + ',"' + dormBuildingName + '","' + searchContent + '")\'><a href="javascript:void(0)" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>';
            let nextNum = page.current === page.pages ? page.current : page.current + 1;
            let nextPage = '<li onclick=\'javascript:adm_load_table(' + nextNum + ',"' + dormBuildingName + '","' + searchContent + '")\'><a href="javascript:void(0)" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>';
            lis += beforePage;
            let begin;
            let end;
            if (page.pages < 10) {
            ...
            for (let i = begin; i <= end; i++) {
                let li = '';
                if (page.current === i) {
                    li = '<li class="active" onclick=\'javascript:adm_load_table(' + i + ',"' + dormBuildingName + '","' + searchContent + '")\'><a href="javascript:void(0)">' + i + '</a></li>';
                } else {
                    li = '<li onclick=\'javascript:adm_load_table(' + i + ',"' + dormBuildingName + '","' + searchContent + '")\'><a href="javascript:void(0)">' + i + '</a></li>';
                }
                lis += li;
            }
            lis += nextPage;
        }
        $('#adm-pagination').html(lis);
        ...
        let table = '<tbody>' +
            ...
            '            </tr>';
        for (let i = 0; i < page.records.length; i++) {
            let manager = page.records[i];
            let tr = '<tr>\n' +
                '                <td>' + manager.dormBuilding.dbname + '</td>\n' +
                '                <td>' + manager.name + '</td>\n' +
                ...
                '                <td>\n' +
                '                    <button type="button" class="btn btn-primary btn-success" data-toggle="modal" ' +
                '                           data-target="#adm-update' + manager.mid + '" onclick="adm_show_update_modal(' + manager.mid + ')">修改</button>\n' +
                '                    <button type="button" class="btn btn-primary btn-danger" data-toggle="modal" ' +
                '                           data-target="#adm-delete' + manager.mid + '" onclick="adm_show_delete_model(' + manager.mid + ' )">删除</button>\n' +
                '                </td>\n' +
                '            </tr>';
            table += tr;
        }
        table += '</tbody>';
        $('#adm-table').html(table);
        tableFormatting();  //表格格式化
    });
}
